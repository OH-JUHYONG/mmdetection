...
def limit_files_to_match(directory1, ext1, directory2, ext2, limit):
    files1 = {f.split('.')[0]
              for f in os.listdir(directory1) if f.endswith(ext1)}
    files2 = {f.split('.')[0]
              for f in os.listdir(directory2) if f.endswith(ext2)}
...
# 파일 제한 함수 호출
limit_files_to_match(images_test_dir, '.tif',
                     annfiles_test_dir, '.json', 110000)  # 남길 파일 갯수 설정
